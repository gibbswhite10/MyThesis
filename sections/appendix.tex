% 参考文献，此处以 MLA 引用格式为例
%行文到此处，按照美赛的惯例是需要插入1-2张海报的，但是我这里并没有制作海报的模板
%这是因为LaTeX生成的学术海报一般较大，为A1或者A0,且内容密度较高，如果贸然将版面改为A4,会导致内容过密,影响阅读体验
%因此，我在main.tex中插入了两页空白用于替代彩页的位置
%当彩页在其他软件中绘制完毕后，导出pdf格式将这两页进行替换即可
%并不是LaTeX不能生成a4格式的海报或者彩页
%只是对于非常自由的排版，其他专业的海报软件如ps，显然能更好的满足需求，所谓“适合的才是最好的”
%------------------------------------------------------------------------------------
...
# Long Long  Long Long  Long Long  Long Long  Long Long Data analysis and visualization# 
...
\begin{tcolorbox}[title=\textbf{Question: Mathematical modeling for fire spread prediction}, colback=blue!2!white, colframe=blue!30!black, fonttitle=\bfseries, breakable]
...
\begin{tcolorbox}[title=\textbf{Question: MATLAB implementation for data visualization}, colback=blue!2!white, colframe=blue!30!black, fonttitle=\bfseries, breakable]
...
\begin{ai_matlab}
%Long Long Long Long Long Long Long Long Long Long Long Long Heat map for temperature distribution
...
\begin{tcolorbox}[title=\textbf{AI Response}, colback=green!2!white, colframe=green!30!black, fonttitle=\bfseries, breakable]
For wildfire data visualization in Python:
...
# Long Long Long Long Long Long Long Long Long Long Long Long Define differential equation (growth phase)
...
# Parameter settings
T_harvest = 365  # Harvest cycle
T_total = 3650   # Total simulation time
dt = 1
...
while t_current < T_total:
    t_next = t_current + T_harvest
    t_span = np.linspace(t_current, t_next, int(T_harvest/dt))
    
    # Integrate one cycle
    sol = odeint(model, state, t_span, args=(params,))
    results.append(sol)
    
    # Apply pulse (harvest event)
    state = sol[-1] # Get state at end of cycle
    state[0] = state[0] * (1 - h) # Crop removal ratio h
